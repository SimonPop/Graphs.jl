@testset "Laplacian" begin
    ...
    for g in test_generic_graphs(g5)
        z = @inferred(laplacian_centrality(g))
        @test round.(z, digits=2) == [0.37, 0.75, 0.75, 0.37]
        @test all(z .<= 1)
    end
    ...
    add_edge!(g5, 1, 2)
    for g in test_generic_graphs(g5)
        z = @inferred(laplacian_centrality(g, normalized=false))
        @test z[1] == z[2] == 4.0
        @test z[3] == z[4] == z[5] == 0.0
    end
    ...
    for g in test_generic_graphs(g0)
        @test_throws "null graph has no centrality defined" laplacian_centrality(g)
    end
    ...
    for g in test_generic_graphs(g_complete)
        z = @inferred(laplacian_centrality(g, vs=[1, 3, 5]))
        @test round(z[1]; digits=2) ==
            round(z[2]; digits=2) ==
            round(z[3]; digits=2) ==
            0.52
